from salmontracker import initUser, initAll, hasJobs, locale, hasWeapon, doesntHaveWeapon, avgStat, grizzcoWeapons
import jsonlines
import requests
import pprint
...
def sortWeapons(path: str, data: str, stat: str) -> None:
    ...
    for weapon in weaponsList:
        ...
        if (hasJobs(withVal[0], withVal[1])) and (hasJobs(withoutVal[0], withoutVal[1])):
            result["name"] = weapon["name"][locale]
            result["value"] = avgStat(withVal[0] + withVal[1], stat) - avgStat(withoutVal[0] + withoutVal[1], stat)
